...
import { X, ChevronsUpDown, LibraryBig } from 'lucide-react'
...
export function SubsetSelectorContent({
  ...
  const [results, setResults] = useState<SelectableSubset[]>([])
  ...
  return (
    <Command shouldFilter={false}>
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1 border-b px-2 py-2">
          {value.map((s) => (
            ...
            >
              <LibraryBig className="h-3 w-3 shrink-0 text-muted-foreground" />
      ...
      <CommandList>
        {!isSearching ? (
          <CommandEmpty>Type to search for subsets.</CommandEmpty>
        ) : searching ? (
          ...
          <CommandGroup>
            {results.map((s) => {
              ...
              return (
                ...
                >
                  <LibraryBig className="mr-2 h-4 w-4 shrink-0 text-muted-foreground" />
