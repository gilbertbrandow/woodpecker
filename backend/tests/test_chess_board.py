...
def test_board_returns_none_for_in_progress() -> None:
    assert compute_attempt_board(STANDARD_FEN, TWO_PLY_SOLUTION, "in_progress", []) is None


def test_board_solved_has_correct_result() -> None:
    result = compute_attempt_board(STANDARD_FEN, TWO_PLY_SOLUTION, "solved", ["d7d5"])
...
def test_board_failed_wrong_move_has_wrong_result() -> None:
    result = compute_attempt_board(STANDARD_FEN, TWO_PLY_SOLUTION, "failed", ["d7d6"])
    assert result is not None
...
def test_board_failed_no_moves_returns_null_fields() -> None:
    result = compute_attempt_board(STANDARD_FEN, TWO_PLY_SOLUTION, "failed", [])
    assert result is not None
...
def test_pgn_returns_none_for_in_progress() -> None:
    assert compute_attempt_pgn(STANDARD_FEN, TWO_PLY_SOLUTION, "in_progress", []) is None


def test_pgn_solved_mainline_contains_moves() -> None:
    result = compute_attempt_pgn(STANDARD_FEN, TWO_PLY_SOLUTION, "solved", ["d7d5"])
    assert result is not None
...
def test_pgn_failed_wrong_move_produces_variation() -> None:
    result = compute_attempt_pgn(STANDARD_FEN, TWO_PLY_SOLUTION, "failed", ["d7d6"])
    assert result is not None
...
def test_pgn_multi_move_solution_solved() -> None:
    result = compute_attempt_pgn(STANDARD_FEN, FOUR_PLY_SOLUTION, "solved", ["d7d5", "d8d5"])
    assert result is not None
    assert result["variation"] is None
    mainline = result["mainline"]
